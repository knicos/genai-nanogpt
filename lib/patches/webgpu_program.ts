...
// Nick: Allow subgroups in WebGPU programs.

import { backend_util, DataType, env, TensorInfo, util } from '@tensorflow/tfjs-core';

import { symbolicallyComputeStrides } from '@tensorflow/tfjs-backend-webgpu/dist/shader_util';
import {
    dataTypeToGPUType,
    getCoordsDataType,
    ...
    typeSnippet,
} from '@tensorflow/tfjs-backend-webgpu/dist/webgpu_program';
...
export interface WebGPUProgram {
    ...
    atomic?: boolean;
    // Whether the program uses subgroups.
    subgroups?: boolean;
...
): GPUComputePipeline | Promise<GPUComputePipeline> => {
...
function makeShader(
    ...
    prefixSnippets.push(`
      ...
      var<private> workgroupId: vec3<u32>;
        ${program.subgroupBuiltins ? 'var<private> subgroupInvocationId: u32;' : ''}
        ${program.subgroupBuiltins ? 'var<private> subgroupSize: u32;' : ''}
    ...
    if (program.pixelsOpType != null) {
        const inoutSnippet =
            program.pixelsOpType === PixelsOpType.FROM_PIXELS
                ? `@group(0) @binding(0) var<storage, read_write> result: array<${dataTypeToGPUType(
                      outputData.dtype,
                      program.outputComponent
                  )}>;`
                : `@group(0) @binding(1) var<storage, read> inBuf : array<${dataTypeToGPUType(
                      inputInfo[0].dtype,
                      program.outputComponent
                  )}>;`;
    ...
    const enableSnippet = program.subgroups ? 'enable subgroups;' : '';
...
function getInputByOutputSnippet(
  ...
  `;
    ...
    if (outRank < 2 && inRank > 0) {
        ...
        if (outRank > 1) {
            ...
            const coordsValues = inputInfo.shape.map((_, i) => `coords.${getCoordsXYZ(i + rankDiff)}`).join(', ');
  ...
  fn ${funcName}Index(globalIndex : i32) -> ${typeSnippet(component)} {
    ...
    ${coordsSnippet}
    return ${typeSnippet(component)}(${texName}[getIndexFromCoords${rankStr}(${unpackedCoordsSnippet}, ${shapeStr})${
        component === 1 ? '' : ` / ${component}`
    }]);
  }
    ...
    const snippet = `
     ${getWorkgroupSizeString(program)}
      fn _start(@builtin(local_invocation_id) LocalId : vec3<u32>,
                ...
                ${program.subgroupBuiltins ? '@builtin(subgroup_invocation_id) SubgroupInvocationId : u32,' : ''}
                ${program.subgroupBuiltins ? '@builtin(subgroup_size) SubgroupSize : u32,' : ''}
        ...
        ${program.subgroupBuiltins ? 'subgroupInvocationId = SubgroupInvocationId' : ''};
        ${program.subgroupBuiltins ? 'subgroupSize = SubgroupSize' : ''};
