...
import { backpropagateGradients } from './tape';
...
interface CustomGradKernelInvocation<T extends Tensor | Tensor[], I extends NamedTensorMap> {
...
function isRegisteredKernelInvocation<T extends Tensor | Tensor[], I extends NamedTensorMap>(
...
class EngineState {
    ...
    activeProfile: ProfileInfo = {
        ...
        peakBytes: 0,
        kernels: [],
        result: null as unknown as TensorContainer,
...
export class Engine implements TensorTracker, DataMover {
    ...
    constructor(public ENV: Environment) {
        ...
        console.log('GenAI Patched Engine Initialized');
    }
        ...
        if (!(backendName in this.registry)) {
            ...
            if (backendName in this.registryFactory) {
                ...
                if (asyncInit) {
                    ...
                    return null as unknown as KernelBackend;
                }
            } else {
                return null as unknown as KernelBackend;
            }
    ...
    findBackendFactory(backendName: string): () => KernelBackend | Promise<KernelBackend> {
        if (!(backendName in this.registryFactory)) {
            return null as unknown as () => KernelBackend | Promise<KernelBackend>;
        }
        return this.registryFactory[backendName].factory;
    }

    registerBackend(backendName: string, factory: () => KernelBackend | Promise<KernelBackend>, priority = 1): boolean {
        if (backendName in this.registryFactory) {
            log.warn(`${backendName} backend was already registered. ` + `Reusing existing backend factory.`);
            return false;
        }
        this.registryFactory[backendName] = { factory, priority };
        console.log('Registered backend', backendName);
        return true;
    }
    ...
    async setBackend(backendName: string): Promise<boolean> {
        ...
        if (this.registry[backendName] == null) {
            this.backendInstance = null as unknown as KernelBackend;
     ...
     */
    private initializeBackend(backendName: string): { success: boolean | Promise<boolean>; asyncInit: boolean } {
            ...
            if (backend && !(backend instanceof KernelBackend) && typeof backend.then === 'function') {
                ...
                const success = backend
                    .then((backendInstance) => {
                        ...
                        this.pendingBackendInit = null as unknown as Promise<boolean>;
                        return true;
                    })
                    .catch((err) => {
                        // Outdated promise. Another backend was set in the meantime.
    ...
    removeBackend(backendName: string): void {
        ...
        if (this.backendName === backendName) {
            this.pendingBackendInit = null as unknown as Promise<boolean>;
            this.backendName = null as unknown as string;
            this.backendInstance = null as unknown as KernelBackend;
        }
    }
    ...
    private initializeBackendsAndReturnBest(): { name: string; asyncInit: boolean } {
        const sortedBackends = this.getSortedBackends();
    ...
    tidy<T extends TensorContainer>(nameOrFn: string | ScopeFn<T>, fn?: ScopeFn<T>): T {
        let name: string = null as unknown as string;
        if (fn == null) {
    ...
    private clone(x: Tensor): Tensor {
        ...
        const y: Tensor = wasPacked
            ? packTensor(ENGINE.runKernel(Identity, { x } as unknown as NamedTensorMap))
            : ENGINE.runKernel(Identity, { x } as unknown as NamedTensorMap);
        const inputs = { x };
        const grad = (dy: Tensor | Tensor[]) => ({
            x: () => {
                const dtype = 'float32';
    ...
    private checkKernelForMemLeak(kernelName: string, numDataIdsBefore: number, outInfos: TensorInfo[]): void {
    ...
    private runKernelFunc<T extends Tensor | Tensor[], I extends NamedTensorMap>(
        kernelParams: RegisteredKernelInvocation<I> | CustomGradKernelInvocation<T, I>
    ): T {
        let outputs: Tensor[];
        let saved: Tensor[] = [];
        ...
        if (isTapeOn) {
            this.addTapeNode(
                ...
                inputs,
                outputs!,
                backwardsFunc as unknown as GradFunc,
                saved,
                attrs ?? {}
            );
        }
        ...
        if (this.state.profiling) {
            this.state.activeProfile.kernels.push({
                ...
                inputShapes: Object.keys(inputs).map((key) =>
                    inputs[key] != null ? inputs[key].shape : null
                ) as number[][],
     ...
     */
    private getTensorsForGradient(kernelName: string, inputs: NamedTensorMap, outputs: Tensor[]): Tensor[] | null {
        ...
        if (gradConfig != null) {
            ...
            const outputTensorsToSave: Tensor[] = outputs.filter((_, i) => outputsToSave[i]);
    ...
    makeTensorFromTensorInfo(tensorInfo: TensorInfo, backend?: KernelBackend): Tensor {
        ...
        if (t.packed && dtype !== 'int32') {
            throw new Error('Only int32 tensors can be packed.');
        }
        this.trackTensor(t, backend ?? this.backend);
        return t;
    }

    makeVariable(initialValue: Tensor, trainable = true, name?: string, dtype?: DataType): Variable {
        name = name || this.nextVariableId().toString();
        if (dtype != null && dtype !== initialValue.dtype) {
            initialValue = initialValue.cast(dtype);
        }
        // NICK: Use PackableVariable
        const v = new PackableVariable(initialValue, trainable, name, this.nextTensorId());
    ...
    async profile(query: () => TensorContainer | Promise<TensorContainer>): Promise<ProfileInfo> {
    ...
    private addTapeNode(
        ...
        const tapeNode: TapeNode = { id: this.state.nextTapeNodeId++, kernelName, inputs, outputs, saved };
    ...
    gradients<T extends Tensor>(
        ...
        return this.tidy('backward', () => {
            ...
            if (this.state.gradientDepth === 0) {
                ...
                this.state.activeTape = null as unknown as TapeNode[];
            }
    ...
    customGrad<T extends Tensor>(f: CustomGradientFunc<T>): (...args: Array<Tensor>) => T {
        ...
        return (...inputs: Tensor[]): T => {
            ...
            const forwardFunc: ForwardFunc<T> = (_, save) => {
                res = f(...[...inputs, save as GradSaveFunc]);
                util.assert(
